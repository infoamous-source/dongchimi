...
const items = [
  ...
  { id: 'reviews', title: '재취업 후기', description: '분야별 재취업 경험담과 정보', icon: '💬', path: '/career/reviews' },
]
...
export default function CareerWorkPage() {
  return (
    <div className="max-w-2xl mx-auto px-4 py-8 animate-fade-in">
      ...
      <h1 className="text-3xl font-extrabold text-dc-text mb-3">💼 일터</h1>
      <p className="text-xl text-dc-text-secondary mb-8">이력서, 자기소개서 작성과 취업 정보</p>

      <div className="flex flex-col gap-4">
        {items.map(item => (
          <Link key={item.id} to={item.path} className="card flex items-center gap-5 hover:shadow-lg transition-shadow">
            <div className="w-16 h-16 rounded-3xl bg-emerald-50 flex items-center justify-center text-3xl shrink-0">{item.icon}</div>
