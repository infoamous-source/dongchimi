...
import { Send, Bot, User, Loader2, RotateCcw, ArrowLeft, Check, Key } from 'lucide-react'
...
export default function CareerAiPage() {
  ...
  const handleRegisterKey = async () => {
    ...
    if (user) await supabase.from('dc_profiles').update({ gemini_api_key: apiKey.trim() }).eq('id', user.id)
  ...
  const handleRemoveKey = async () => {
    clearStoredApiKey()
    if (user) await supabase.from('dc_profiles').update({ gemini_api_key: null }).eq('id', user.id)
  ...
  const sendMessage = async (text: string) => {
    ...
    setLoading(true)
    try {
      const response = await askTutor(text.trim(), undefined, [...messages, userMsg])
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-8 animate-fade-in">
        <Link to="/career" className="inline-flex items-center gap-2 text-dc-text-secondary mb-6 text-lg font-bold">
          <ArrowLeft size={24} /><span>중장년층 프로그램</span>
        </Link>
  ...
  if (messages.length === 0) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-8 animate-fade-in flex flex-col min-h-[calc(100vh-8rem)]">
        <Link to="/career" className="inline-flex items-center gap-2 text-dc-text-secondary mb-4 text-lg font-bold">
          <ArrowLeft size={24} /><span>중장년층 프로그램</span>
        </Link>
        <div className="text-center mb-6">
          <div className="w-20 h-20 rounded-full bg-dc-green mx-auto mb-4 flex items-center justify-center"><Bot size={40} className="text-white" /></div>
          <h1 className="text-3xl font-extrabold text-dc-text mb-2">AI비서</h1>
          <div className="inline-flex items-center gap-2 bg-dc-green-bg border-2 border-dc-green-pale rounded-full px-5 py-2">
            <Check size={20} className="text-dc-green" /><span className="text-lg font-bold text-dc-green">API 등록완료</span>
          </div>
          <p className="text-xl text-dc-text-secondary mt-3">취업, 디지털, 생활 뭐든 물어보세요!</p>
          {!showKeyInput ? (
            <button onClick={() => setShowKeyInput(true)} className="text-base text-dc-text-muted mt-2 underline">API 키 변경</button>
          ) : (
  ...
  return (
    <div className="max-w-2xl mx-auto flex flex-col h-[calc(100vh-8rem)]">
      <div className="px-4 py-4 flex items-center justify-between border-b border-gray-100">
        ...
        <button onClick={() => setMessages([])} className="flex items-center gap-2 text-dc-text-muted text-lg font-bold px-3 py-2 rounded-xl hover:bg-gray-100">
